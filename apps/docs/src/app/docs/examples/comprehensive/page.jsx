...
const APP_CODE = `import { useState, useCallback, useRef } from 'react';
import {
...
function DashedEdge({ sourceX, sourceY, targetX, targetY, sourcePosition, targetPosition, markerEnd }) {
  ...
  return <BaseEdge path={path} markerEnd={markerEnd} style={{ strokeDasharray: '6 3', stroke: '#94a3b8' }} />;
}
        ...
        <div style={{
          ...
          {PALETTE.map((item) => (
            <div key={item.type} draggable onDragStart={(e) => onDragStart(e, item.type)}
              style={{ padding: '5px 10px', border: \`1px solid \${item.border}\`, borderRadius: 5,
                cursor: 'grab', textAlign: 'center', background: item.color, fontSize: 12, fontWeight: 500 }}>
              {item.label}
            </div>
          ))}
        </div>
...
const tableOfContents = [
  ...
  { href: '#custom-nodes', label: 'Custom nodes' },
  { href: '#custom-edges', label: 'Custom edges' },
  ...
  return (
    <>
      <div className="page-content">
        ...
        <p>
          A full-featured example combining custom nodes, custom edges, drag-and-drop,
          and all built-in components in a single interactive canvas.
        </p>

        <h2 id="live-demo">Live demo</h2>
        <ExampleBlock preview={<ComprehensiveExample />} files={files} />

        <h2 id="custom-nodes">Custom nodes</h2>
        <p>This example includes three custom node types:</p>
        <ul>
          <li><strong>CardNode</strong> — A rich card with subtitle, description, multiple handles (top/left/right/bottom), and a NodeToolbar with edit/delete actions</li>
          <li><strong>CircleNode</strong> — A circular decision node with handles on all four sides</li>
          <li><strong>StatusNode</strong> — A status indicator node with a colored dot (success, warning, error, info)</li>
        </ul>
        <p>
          Register custom nodes by passing a <code>nodeTypes</code> object to <code>InfiniteCanvas</code>.
          Each key maps to a React component that receives <code>data</code> and <code>selected</code> props.
          Use the <code>Handle</code> component to define connection points.
        </p>

        <h2 id="custom-edges">Custom edges</h2>
        <p>Two custom edge types are included:</p>
        <ul>
          <li><strong>ButtonEdge</strong> — A bezier edge with an interactive button label rendered via <code>EdgeLabelRenderer</code></li>
          <li><strong>DashedEdge</strong> — A smooth-step edge with a dashed stroke style</li>
        </ul>
        <p>
          Register custom edges with the <code>edgeTypes</code> prop. Each component receives
          source/target coordinates and positions, then uses path utilities like <code>getBezierPath</code> or <code>getSmoothStepPath</code> with <code>BaseEdge</code>.
        </p>

        <h2 id="drag-and-drop">Drag and drop</h2>
        <p>
          The sidebar palette uses native HTML drag-and-drop. Each draggable item stores its node type,
          and the <code>onDrop</code> handler creates a new node at the drop position with type-specific defaults.
        </p>
        <ol>
          <li>Track the dragged type in state via <code>onDragStart</code></li>
          <li>Allow dropping with <code>onDragOver</code> + <code>preventDefault</code></li>
          <li>On drop, create a new node with <code>setNodes</code> using the offset position</li>
          <li>Connect new nodes by dragging between handles</li>
        </ol>
      </div>
